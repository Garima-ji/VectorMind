...
export default function Home() {
  ...
  const handleSearch = async (searchQuery?: string) => {
    ...
    try {
      ...
      console.error(err)
      setError("Failed to fetch search results. Verify that your backend container is running.")
    } finally {
      setLoading(false)
    }
  }
  ...
  const clusterColors = [
    { fill: "#6366F1", text: "text-indigo-600", bg: "bg-indigo-50 dark:bg-indigo-950/30", label: "Graphics & Computers" },
    { fill: "#10B981", text: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-950/30", label: "Medicine & Health" },
    { fill: "#8B5CF6", text: "text-violet-600", bg: "bg-violet-50 dark:bg-violet-950/30", label: "Space & Science" },
    { fill: "#F59E0B", text: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-950/30", label: "Hockey & Sports" },
    { fill: "#EF4444", text: "text-rose-600", bg: "bg-rose-50 dark:bg-rose-950/30", label: "Politics & Law" }
  ]

  // Render highlighted search queries matching input
  const renderHighlighted = (text: string, kwQuery: string) => {
    if (!kwQuery) return text
    const words = kwQuery.split(" ").filter(w => w.length > 2)
    let result = text
    words.forEach(word => {
      const regex = new RegExp(`(${word.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')})`, "gi")
      result = result.replace(regex, `<mark class="bg-amber-100 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 font-semibold px-1 rounded-sm">$1</mark>`)
    })
    return result
  }
  ...
  const renderTSNEScatter = () => {
    ...
    return (
      <div className="relative border border-gray-200 dark:border-gray-800 rounded-2xl bg-white dark:bg-gray-900 p-6 flex flex-col items-center">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full max-w-xl h-auto overflow-visible cursor-crosshair">
          {/* Grid lines */}
          <line x1={padding} y1={height/2} x2={width-padding} y2={height/2} stroke="#E5E7EB" strokeWidth="1" strokeDasharray="3" className="dark:stroke-gray-800" />
          <line x1={width/2} y1={padding} x2={width/2} y2={height-padding} stroke="#E5E7EB" strokeWidth="1" strokeDasharray="3" className="dark:stroke-gray-800" />
          
          {clusterPoints.map((point) => {
            const cx = scaleX(point.x)
            const cy = scaleY(point.y)
            const color = clusterColors[point.cluster % clusterColors.length].fill
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-4 text-xs font-medium">
          {clusterColors.map((color, idx) => (
            <div key={idx} className="flex items-center gap-2">
              <span className="h-3.5 w-3.5 rounded-full inline-block" style={{ backgroundColor: color.fill }}></span>
        ...
        {activeTab === "search" && (
          <div className="space-y-8">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md">
                      ...
                      {suggestions.map((s, idx) => (
                        ...
                        >
                          <span className="text-gray-800 dark:text-gray-200 font-medium text-sm flex items-center gap-2">
                            <TrendingUp className="h-3.5 w-3.5 text-indigo-500" /> {s}
                          </span>
                        </div>
                      ))}
                    </div>
              ...
              {hasSearched && expandedQuery && expandedQuery.toLowerCase() !== query.toLowerCase() && (
                <div className="mt-4 px-4 py-2.5 bg-indigo-50/55 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/30 rounded-xl flex items-center gap-2.5 text-xs text-indigo-700 dark:text-indigo-300">
                  ...
                  <span><strong>Query expanded for recall:</strong> {expandedQuery}</span>
                </div>
              )}
                ...
                <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800 flex flex-col md:flex-row justify-between gap-6">
                  {recentSearches.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5"><History className="h-3 w-3" /> Recent Queries</p>
            ...
            {!loading && hasSearched && topResult && (
              <div className={`space-y-8 transition-opacity duration-300 ${showResults ? 'opacity-100' : 'opacity-0'}`}>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 text-xs font-bold text-gray-400 px-2">
                  ...
                  <div className="flex gap-4">
                    <span>Latency: <strong className="text-emerald-500">{processingTime}s</strong></span>
                    <span>Cache: <strong className={cacheStatus === "HIT" ? "text-indigo-500" : "text-amber-500"}>{cacheStatus}</strong></span>
                    <span>Cluster: <strong className="text-violet-500">#{cluster}</strong></span>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-tr from-indigo-500/5 to-purple-500/5 dark:from-indigo-950/20 dark:to-purple-950/20 border-2 border-indigo-500/20 rounded-3xl p-8 shadow-lg relative overflow-hidden">
                  ...
                  <h3 className="text-2xl font-bold mb-4 bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-400 dark:to-purple-400 bg-clip-text text-transparent flex items-center gap-2">
                    <Cpu className="h-6 w-6 text-indigo-500" /> AI Grounded Answer
                  </h3>
                  ...
                  {sources.length > 0 && (
                    <div className="pt-4 border-t border-gray-200/50 dark:border-gray-850 flex flex-wrap items-center gap-3">
                      <span className="text-xs font-bold text-gray-400 uppercase tracking-wider"><BookOpen className="h-3.5 w-3.5 inline mr-1" /> Referenced Sources:</span>
                      {sources.map((src, idx) => (
                        <span key={idx} className="text-xs font-semibold bg-gray-150 dark:bg-gray-800 text-gray-600 dark:text-gray-300 px-3 py-1.5 rounded-lg border border-gray-200/20 shadow-sm">
                          {src}
                        </span>
                      ))}
                    </div>
                ...
                <div className="space-y-4">
                  <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 flex items-center gap-2">
                    <Database className="h-5.5 w-5.5 text-indigo-500" /> Ranked Context Documents ({results.length})
                  </h2>

                  <div className="space-y-4">
                    {results.map((result, index) => {
                      const isExplaining = activeExplanation === index
                      const colorSet = clusterColors[result.cluster % clusterColors.length]
                        ...
                        >
                          ...
                          <div className="flex justify-between items-center mb-4">
                            <div className="flex items-center gap-2.5">
                              ...
                              <span className={`text-xs font-bold uppercase px-2.5 py-1 rounded-full border border-transparent ${colorSet.bg} ${colorSet.text}`}>
                                {colorSet.label}
                              </span>
                            </div>
                            ...
                            <div className="flex gap-4 text-xs font-bold">
                              ...
                              {result.rrf_score !== undefined && (
                                <span className="text-purple-600 dark:text-purple-400">
                                  RRF Rank Score: {roundVal(result.rrf_score, 3)}
                                </span>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="pt-2 border-t border-gray-100 dark:border-gray-850">
                            <button
                              onClick={() => setActiveExplanation(isExplaining ? null : index)}
                            ...
                            >
                              <Info className="h-3.5 w-3.5" /> {isExplaining ? "Hide explanation" : "Why was this document retrieved?"}
                            </button>
                            ...
                            {isExplaining && result.explanation && (
                              <div className="mt-3 p-4 bg-gray-50 dark:bg-gray-950 rounded-xl border border-gray-200/50 dark:border-gray-800 text-xs text-gray-600 dark:text-gray-400 leading-relaxed animate-fadeIn">
                                <strong>Retrieval Logic:</strong> {result.explanation}
                              </div>
                            )}
                          </div>
                        </div>
            ...
            {!hasSearched && !loading && (
              <div className="rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-800 bg-white/50 dark:bg-gray-900/35 p-16 text-center shadow-sm">
                ...
                <h3 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Intelligent Retrieval System</h3>
                <p className="text-gray-500 dark:text-gray-400 mt-2 max-w-md mx-auto">
                  Type a query above to query document embeddings, apply BM25 hybrid indexing, and run cross-encoder LLM reranking.
                </p>
              </div>
            )}
          </div>
            ...
            <div className="md:col-span-1 space-y-6">
              <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
                  This 2D visualization projects the 384-dimensional document embeddings down to a 2D space using **t-Distributed Stochastic Neighbor Embedding (t-SNE)**.
                </p>
                ...
                <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800 space-y-4">
                  ...
                  >
                    ...
                    {reindexing ? "Reindexing Corpus..." : "Trigger Reindex Corpus"}
                  </button>
                  
                  {reindexSuccess && (
                    <p className="text-xs font-bold text-emerald-500 text-center flex items-center justify-center gap-1.5"><CheckCircle className="h-3.5 w-3.5" /> Reindexed successfully!</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md min-h-48 flex flex-col justify-between">
                {hoveredPoint ? (
                  <div className="space-y-3">
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Document Details</p>
                    <span className={`text-[10px] font-bold uppercase px-2.5 py-0.5 rounded-full inline-block ${clusterColors[hoveredPoint.cluster % clusterColors.length].bg} ${clusterColors[hoveredPoint.cluster % clusterColors.length].text}`}>
                      {clusterColors[hoveredPoint.cluster % clusterColors.length].label}
                    </span>
                    ...
                    <div className="flex justify-between text-[10px] font-bold text-gray-400 pt-2">
                      <span>Coordinates: ({hoveredPoint.x}, {hoveredPoint.y})</span>
                      <span>Membership probability: {Math.round(hoveredPoint.probability * 100)}%</span>
                    </div>
                  </div>
                ) : (
                  <div className="h-full flex items-center justify-center text-center py-12 text-gray-400">
                    <p className="text-xs font-semibold">Hover over a cluster point coordinate to inspect document details.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="md:col-span-2">
              {clusterLoading ? (
                <div className="w-full border border-gray-200 dark:border-gray-800 rounded-3xl bg-white dark:bg-gray-900 h-96 flex flex-col items-center justify-center gap-3 animate-pulse shadow-sm">
                  ...
                  <p className="text-sm font-semibold text-gray-400">Calculating t-SNE projections...</p>
                </div>
              ) : (
                renderTSNEScatter()
              )}
            ...
            {!analyticsLoading && analyticsData && (
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-800 dark:text-gray-100">{analyticsData.total_queries}</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-800 dark:text-gray-100">{Math.round(analyticsData.cache_hit_rate * 100)}%</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-800 dark:text-gray-100">{analyticsData.average_latency_ms} ms</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {/* Top Searches */}
                  <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md">
                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><TrendingUp className="h-4 w-4 text-indigo-500" /> Top Queries</h3>
                    {analyticsData.top_queries.length > 0 ? (
                      <div className="space-y-3">
                        {analyticsData.top_queries.map((item: any, idx: number) => (
                          <div key={idx} className="flex justify-between items-center bg-gray-50 dark:bg-gray-950 px-4 py-3 rounded-xl">
                            <span className="text-xs font-bold text-gray-850 dark:text-gray-300">{item.query}</span>
                            <span className="text-xs font-semibold bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 px-3 py-1 rounded-md">{item.count} searches</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-xs font-semibold text-gray-400 text-center py-12">No search metrics logged yet.</p>
                    )}
                  </div>

                  {/* Cluster distributions */}
                  <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md">
                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Map className="h-4 w-4 text-indigo-500" /> Cluster Distribution</h3>
                    <div className="space-y-4">
                      {clusterColors.map((color, idx) => {
                        const count = analyticsData.cluster_distribution[idx] || 0
                        const total = Object.values(analyticsData.cluster_distribution).reduce((a: any, b: any) => a + b, 0) as number
                        ...
                        return (
                          <div key={idx} className="space-y-1">
                            ...
                            <div className="h-3 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                              <div className="h-full rounded-full transition-all duration-300" style={{ backgroundColor: color.fill, width: `${pct}%` }}></div>
                            </div>
                          </div>
                        )
                      })}
                ...
                {analyticsData.recent_latencies.length > 0 && (
                  <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md">
                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Cpu className="h-4 w-4 text-indigo-500" /> Recent Latency Distribution</h3>
                    <div className="h-28 w-full flex items-end gap-1.5 pt-4">
                      {analyticsData.recent_latencies.map((lat: number, idx: number) => {
                        const maxLat = Math.max(...analyticsData.recent_latencies, 0.1)
                    ...
                    <div className="flex justify-between text-[10px] font-bold text-gray-400 pt-2 border-t border-gray-100 dark:border-gray-800 mt-2">
                      ...
                      <span>Latest Query: {roundVal(analyticsData.recent_latencies[analyticsData.recent_latencies.length - 1] * 1000, 1)}ms</span>
                    </div>
                  </div>
                )}
              </>
          ...
          <div className="space-y-8 animate-fadeIn">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="space-y-1">
                <h3 className="text-xl font-bold flex items-center gap-2">
                  <Sliders className="h-5 w-5 text-indigo-500" /> Retrieval Evaluation Framework
                </h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
                  Benchmarks retrieval metrics (NDCG, Precision, Recall, MRR) over a ground truth evaluation set.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {evaluationLoading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}
                  Run Benchmark
                </button>
              </div>
            </div>
            ...
            {evaluationLoading ? (
              <div className="w-full border border-gray-200 dark:border-gray-800 rounded-3xl bg-white dark:bg-gray-900 h-80 flex flex-col items-center justify-center gap-3 animate-pulse shadow-sm">
                <RefreshCw className="h-8 w-8 animate-spin text-indigo-500" />
                <p className="text-sm font-semibold text-gray-400">Running evaluation test suite...</p>
              </div>
            ) : (
              evaluationReport && (
                <>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 text-center">
                      <p className="text-2xl font-black text-indigo-600 dark:text-indigo-400">{Math.round(evaluationReport.ndcg_at_k * 100)}%</p>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1">NDCG@{evaluationReport.k}</p>
                    </div>

                    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 text-center">
                      <p className="text-2xl font-black text-purple-600 dark:text-purple-400">{Math.round(evaluationReport.precision_at_k * 100)}%</p>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1">Precision@{evaluationReport.k}</p>
                    </div>

                    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 text-center">
                      <p className="text-2xl font-black text-emerald-600 dark:text-emerald-400">{Math.round(evaluationReport.recall_at_k * 100)}%</p>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1">Recall@{evaluationReport.k}</p>
                    </div>

                    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 text-center">
                      <p className="text-2xl font-black text-amber-600 dark:text-amber-400">{Math.round(evaluationReport.mrr * 100)}%</p>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1">Mean Reciprocal Rank</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-6 shadow-md overflow-x-auto">
                    <h3 className="text-lg font-bold mb-4">Detailed Query Report</h3>
                    <table className="w-full text-left text-xs divide-y divide-gray-100 dark:divide-gray-800 font-medium">
                      ...
                      <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {evaluationReport.query_reports.map((report: any, idx: number) => (
                          <tr key={idx}>
                            <td className="py-3.5 pr-4 text-gray-800 dark:text-gray-200 font-semibold">{report.query}</td>
                            <td className="py-3.5 text-center text-gray-400">{report.relevant_count} docs</td>
                            <td className="py-3.5 text-center text-indigo-600 font-bold">{Math.round(report.precision_at_k * 100)}%</td>
                            <td className="py-3.5 text-center text-emerald-600 font-bold">{Math.round(report.recall_at_k * 100)}%</td>
                            <td className="py-3.5 text-center text-amber-600 font-bold">{Math.round(report.mrr * 100)}%</td>
                            <td className="py-3.5 text-center text-purple-600 font-bold">{Math.round(report.ndcg_at_k * 100)}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
...
function roundVal(num: number, decimals = 2) {
